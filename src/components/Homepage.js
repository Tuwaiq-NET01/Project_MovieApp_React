...
export default function Homepage() {
  ...
  const getmovie= () =>{
    ...
    const movieList = movie.map((item, index) => {
        return <MovieCards key={index}  Poster =  {<img src = {"https://image.tmdb.org/t/p/w500"+item.poster_path} />}  Title ={item.original_title} Year={item.release_date} imdbID={"imdb ID: "+item.id} Type={item.original_language} id={item.id} />
    })
